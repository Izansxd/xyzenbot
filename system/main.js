...
const {
	...
	writeExifImg,
	writeExifVid,
} = require("../lib/assets/watermark");
const {
	smsg,
	getBuffer
} = require("../lib/assets/func");
const { Low, JSONFile } = require('@xyzendev/lowdb');
const cloudDBAdapter = require('../lib/assets/cloudDBAdapter');
const _ = require('lodash');
const yargs = require('yargs/yargs');
const mongoDB = require('../lib/assets/mongoDB');
const {
...
cfonts.say("XYZEN-BOT", {
    font: 'tiny',
	align: 'center',
   	colors: ['system']
});
cfonts.say('Github: https://github.com/xyzendev/xyzen-bot', {
	colors: ['system'],
   	font: 'console',
   	align: 'center'
})
...
async function startAdrian() {
	...
	client.ev.on("messages.upsert", async (chatUpdate, msg) => {
		try {
			...
			require("./client")(client, m, chatUpdate, store);
		} catch (err) {
			console.log(err)
		}
	});
